...
// --- CRUD TICKETS ---
app.all('/rest/v1/tickets', async (request, reply) => {
    ...
    try {
        ...
        if (method === 'POST') {
            ...
            await pool.query(
                ...
                [userId, 'insert', 'tickets', JSON.stringify(request.body), request.ip]
        ...
        if (method === 'PATCH') {
            ...
            await pool.query(
                'INSERT INTO audit_logs (user_id, action, resource, new_value, ip_address) VALUES ($1, $2, $3, $4, $5)',
                [userId, 'update', 'tickets', JSON.stringify({ id, ...request.body }), request.ip]
            ).catch(e => console.error('Audit Error:', e));
